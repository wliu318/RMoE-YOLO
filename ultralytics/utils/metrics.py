...
def save_pr_data(px, py, ap, save_path, names={}):
    """安全保存 PR 数据，处理维度不匹配问题"""
    py = np.stack(py, axis=1)
    n_classes = py.shape[0]
    with open(save_path, 'w', newline='') as f:
        writer = csv.writer(f)
        header = ["recall"] + \
                 [f"{names[i]} {ap[i, 0]:.3f}" for i in range(n_classes)] + \
                 [f"mAP50_{ap[:, 0].mean():.3f} "]
        writer.writerow(header)

        for y in enumerate(py.T):
            row = [px] + list(y)
            writer.writerow(row)
        row = [px] + list(py.mean(0))
        writer.writerow(row)

    # except Exception as e:
    #     print(f"保存 PR 数据失败: {e}")
    #     raise
...
def plot_pr_curve(px, py, ap, save_dir=Path("pr_curve.png"), names={}, on_plot=None):
    ...
    # 1. 保存 PR 数据到 CSV
    pr_data = {"Recall (px)": px}  # 初始化 DataFrame，第一列是 px (Recall)
    for i, y in enumerate(py.T):
        pr_data[f"{names[i] if i < len(names) else i}: {ap[i, 0] if i < len(names) else i}"] = y
    pr_data["mAP50"] = py.mean(1)
    df = pd.DataFrame(pr_data)
    csv_path = save_dir.with_suffix(".csv")  # 例如 pr_curve.png → pr_curve.csv
    df.to_csv(csv_path, index=False)

    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            ax.plot(px, y, linewidth=1.2, label=f"{names[i]}: {ap[i, 0]:.1f}")  # plot(recall, precision)
    else:
        ax.plot(px, py, linewidth=1.2, color="grey")  # plot(recall, precision)

    ax.plot(px, py.mean(1), linewidth=1.5, color="blue", label=f"mAP50: {ap[:, 0].mean():.1f}")
    ax.set_xlabel("Recall (%)", fontdict={'fontsize': 14, 'fontfamily': 'Times New Roman', 'weight': 'bold'})
    ax.set_ylabel("Precision (%)", fontdict={'fontsize': 14, 'fontfamily': 'Times New Roman', 'weight': 'bold'})
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ...
    ax.legend(loc="lower left", prop={
                ...
                'size': 8,}, frameon=True, framealpha=0.8, edgecolor='black')
...
def ap_per_class(
    ...
    if plot:
        ...
        # save_pr_data(x, prec_values, ap,  save_dir / f"{prefix}PR_data.csv", names)
