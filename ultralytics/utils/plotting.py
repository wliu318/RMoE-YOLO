...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            if rotated:
                ...
                p1 = (box[0], box[1])
                self.draw.rectangle(box, width=self.lw_box, outline=color)  # box
            if label:
            ...
            if rotated:
                ...
                cv2.rectangle(self.im, p1, p2, color, thickness=self.lw_box, lineType=cv2.LINE_AA)
            if label:
    ...
    def rectangle(self, xy, fill=None, outline=None, width=3):
...
def plot_images(
    ...
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        # print(f"lw= {width}")
        # annotator.draw.draw.draw_rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=20 )
